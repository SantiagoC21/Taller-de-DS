...
def mostrar_graficas():
    ...
    ax1.plot(weaks, df_antes['Cantidad de Fallas Mecanicas'], label="Cantidad de Fallas Mecanicas", color="blue")
    ax2 = ax1.twinx()
    ax2.plot(weaks, df_antes['Discrepancia'], label="Discrepancia", color="red")
    ax1.set_xlabel("Semana")
    ax1.set_ylabel("Cantidad de Fallas Mecanicas", color="blue")
    ax2.set_ylabel("Discrepancia", color="red")
    ax1.set_title("Cantidad de Fallas Mecanicas vs Discrepancia (Escenario Inestable)")
    ...
    ax1.plot(df["Time"], df["Cantidad de Fallas Mecanicas"], label="Cantidad de Fallas Mecanicas", color="blue")
    ax2 = ax1.twinx()
    ax2.plot(df["Time"], df["Discrepancia"], label="Discrepancia", color="red")
    ax1.set_ylabel("Cantidad de Fallas Mecanicas", color="blue")
    ax2.set_ylabel("Discrepancia", color="red")
    ax1.set_xlabel("Semana")
    ax1.set_title("Cantidad de Fallas Mecanicas vs Discrepancia (Mejorado)")
...
def mostrar_tablas():
    ...
    texto_tablas.insert(tk.END, f"{'Semana':<10}{'Cantidad de Fallas Mecanicas':<20}{'Discrepancia':<20}\n")
    texto_tablas.insert(tk.END, "-"*50 + "\n")

   
    for index, row in df_antes.iterrows():
        texto_tablas.insert(tk.END, f"{row['Semana']:<10}{row['Cantidad de Fallas Mecanicas']:<20}{row['Discrepancia']:<20}\n")
    ...
    for index, row in df.iterrows():
        texto_tablas.insert(tk.END, f"{row['Time']:<10}{row['Cantidad de Fallas Mecanicas']:<20}{row['Discrepancia']:<20}\n")
...
ventana = tk.Tk()
ventana.title("Análisis de Sistema de Satisfacción - Cantidad de Fallas Mecanicas")
