...
def mostrar_graficas():
    ...
    ax1.plot(df["Time"], df["Disponibilidad de Flota"], label="Disponibilidad de Flota", color="blue")
    ax2 = ax1.twinx()
    ax2.plot(df["Time"], df["Discrepancia"], label="Discrepancia", color="red")
    ax1.set_ylabel("Disponibilidad de Flota", color="blue")
    ax2.set_ylabel("Discrepancia", color="red")
    ax1.set_xlabel("Semana")
    ax1.set_title("Disponibilidad de Flota vs Discrepancia (Mejorado)")
...
def mostrar_tablas():
    ...
    texto_tablas.insert(tk.END, f"{'Semana':<10}{'Disponibilidad de Flota':<20}{'Discrepancia':<20}\n")
    texto_tablas.insert(tk.END, "-"*50 + "\n")

   
    for index, row in df_antes.iterrows():
        texto_tablas.insert(tk.END, f"{row['Semana']:<10}{row['Disponibilidad de Flota']:<20}{row['Discrepancia']:<20}\n")
    ...
    for index, row in df.iterrows():
        texto_tablas.insert(tk.END, f"{row['Time']:<10}{row['Disponibilidad de Flota']:<20}{row['Discrepancia']:<20}\n")
...
ventana = tk.Tk()
ventana.title("Análisis de Sistema de Satisfacción - Disponibilidad de flota")
