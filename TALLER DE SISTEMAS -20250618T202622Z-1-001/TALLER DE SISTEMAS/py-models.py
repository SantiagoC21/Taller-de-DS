...
def transferir_datos_modelo_a_sqlite(modelo_nombre, info, conn, cur):
    ...
    # Crear query SQL con placeholders
...
def crear_tablas(cur, conn):
    for modelo, info in modelos_vensim_info.items():
...
class DataViewer(QWidget):
    ...
    def mostrar_datos_y_grafico(self):
        ...
        if len(tables) == 0:
            print("BD VACIA, INSERTANDO DATOS...")

            crear_tablas(cursor, conn)
            for nombre, info in modelos_vensim_info.items():
                transferir_datos_modelo_a_sqlite(nombre, info, conn, cursor)
            ...
            print("BD IDENTIFICADA, GUARDANDO DATOS")
            df = pd.read_sql_query(f"SELECT {', '.join(columnas)} FROM {modelo}", conn)
            conn.close()
